...
SELECT DISTINCT A.RESIDENCE_ID, A.BASE_PRICE, A."TITLE", (B.IMAGE).IMAGE, C.CITY, D.NATIONAL_CODE,
	...
	WHERE A.RESIDENCE_TYPE = 'APARTMENT' AND A.RESIDENCE_ID = B.RESIDENCE_ID AND B.IMAGE_ID = 1 AND
	...
	AND EXISTS (SELECT AMENITIES.* FROM AMENITIES WHERE A.RESIDENCE_ID = AMENITIES.RESIDENCE_ID AND AMENITIES.AMENITY = 'INTERNET')
	AND EXISTS (SELECT AMENITIES.* FROM AMENITIES WHERE A.RESIDENCE_ID = AMENITIES.RESIDENCE_ID AND AMENITIES.AMENITY = 'PARKING')
	ORDER BY A.BASE_PRICE ASC LIMIT 20 OFFSET 0;